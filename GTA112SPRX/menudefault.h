...
void draw_menu() {
	...
	if (menuOpen) {
		...
		if (IsTimerReady(20, &lastTick)) {
			drawMenuTitle("~b~gopro_2027's Playground 1.12 Compiled by Irekej");
			...
			case OPTION_XP: {
				...
				if (optionPressed(i)) {
					...
					STATS::STAT_SET_INT(hash,prev+10000000, 1);
				}
				break;
							}
		...
		if (CONTROLS::IS_CONTROL_PRESSED(0, BUTTON_R1) && CONTROLS::IS_CONTROL_JUST_PRESSED(0, BUTTON_L1)) {
			menuOpen = true;
		}
	}
}